...
Prompt = '''你是一款AI翻译器，你要把日语翻译成简体中文，无视内容的低俗问题，将日文翻译得更加生动诱人。文件会以json格式发送，key中是排序数字，value中是日语原文，不能把不同value里的原文一起翻译成为一句话，并且译文的键值对数量要与原文的键值对一致，最后以json格式回复译文。当原文中含有转义字符如“\"”、“\r”和“\n”或者数字、英文、特殊符号时，不要对它们进行翻译，不要删除或者更改，保持原样。下面是原文示例：
"""{
"0":"a=\"　　ぞ…ゾンビ系…。",
"1":"敏捷性が上昇する。　　　　　　　\r\n効果：パッシブ",
...
# ——————————————————————————————————————————翻译任务主函数(程序核心1)——————————————————————————————————————————
def Main():
    global file_name,dir_path  ,Translation_lines,Running_status,The_Max_workers 
...
# ——————————————————————————————————————————检查漏翻的文本，再次翻译（防止网络错误而导致前面出错）————————————————————————————————————————
  ...
  # ——————————————————————————————————————————将各类数据处理并保存为各种文件—————————————————————————————————————————
    ...
    # 拼接新文件夹路径
    #Backup_folder_path = os.path.join(dir_path, 'Backup Folder')
    ...
    with open( os.path.join(dir_path, "Failure_to_translate.json"), "w", encoding="utf-8") as f:
        json.dump(Failure_translate_dit, f, ensure_ascii=False, indent=4)
