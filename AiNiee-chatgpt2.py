...
Pay_RPM_limit = 3500        # 付费用户速率限制每分钟请求数
Pay_TPM_limit = 90000    # 付费用户速率限制每分钟token数，2tokens大概一个汉字，发送和接受的信息都算作一起
...
if Account_Type == 1 :
    ...
    The_Max_Tokens = Free_TPM_limit                  #每分钟最大tokens容量
else :
    The_RPM_limit =  60 / Pay_RPM_limit           
    The_TPM_limit =  Pay_TPM_limit / 60
    The_Max_Tokens = Pay_TPM_limit

#根据账号类型，设定请求限制
api_request = APIRequest(The_RPM_limit)
api_tokens = TokenBucket(tokens_limit_per, The_TPM_limit)
...
def make_request():
    ...
    #改截取文本subset_mid的键为从0到截取长度结束的数字序号，因为AI酱对一万以上的数字排列不是很理解，回复又慢又容易出错
    ...
    tokens_consume = num_tokens_from_messages(messages, model = "gpt-3.5-turbo-0301")  #计算该信息在openai那里的tokens花费
    ...
    print(f"\n--------------------------------------------------------------------------------------")
    print(f"\n\033[1;32mSuccess:\033[0m 翻译已完成 {percent:.2f}%---------------------------------------")
    print(f"\n--------------------------------------------------------------------------------------\n")
