...
def make_request():
    ...
    # ——————————————————————————————————————————对AI回复内容进行各种处理和检查——————————————————————————————————————————
            ...
            if((response_content.count('"')  !=  A) or (response_content.count(':')  !=  B) ):    
                ...
                #检查回答错误次数，如果达到限制，则跳过该句翻译。
                Wrong_answer_count = Wrong_answer_count + 1
                print("\033[1;31mError:\033[0m 当前任务错误次数:",Wrong_answer_count,"到达20次后自动放弃该任务\n")
                if Wrong_answer_count >= 20 :
                    lock2.acquire()  # 获取锁
                    Failure_translate_str = Failure_translate_str + '\n' + subset_mid_str#存储未能成功翻译文本
                    result_str = result_str + '\n' + subset_mid_str #将原文拼接回去      
    ...
    print(f"\n--------------------------------------------------------------------------------------")
    print(f"\n\033[1;32mSuccess:\033[0m 翻译已完成：{percent:.2f}%               已花费费用：{money_used:.4f}＄")
    print(f"\n--------------------------------------------------------------------------------------\n")
...
with open("Failure_to_translate.json", "w", encoding="utf-8") as f:
