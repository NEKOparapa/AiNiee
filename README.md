...
*  SakuraLLM配置:
    ...
    4.部署好模型后，获取接口地址，如：http://127.0.0.1:6006，填入请求地址栏中，注意，接口地址中不要包含`/v1`，否则会报错<br>
...
*   配置示例:<br>
    ...
    >`最大线程数`: 请根据电脑自身情况设置，线程数越大，越容易吃满速率限制，翻译速度越快。最大线程数设置上限为999，建议设置100以内<br>

    >`错误重翻最大次数限制`: 就是一段文本，出现错误回复时，最多允许重复翻译的次数<br>  
    
    >`保留换行符`: 在翻译前将换行符替换成特殊字符，再进行翻译，AI仍会吞符号，不是百分百保留。<br>
...
* ` 自定义模型`
  >在代理平台选择相应的代理类型，并在模型选择的下拉框中进行编辑，可以自定义组合发送格式与模型
...
* 【如何反馈自己在使用中遇到的问题】————————将终端窗口（黑黑的那个框框）的内容完整截图下来，里面有程序运行日志，还有软件界面设置截图，然后将问题描述清晰带上截图到群里或者issue提问。当进一步排除问题，需要到原文本或者翻译后文本时，请压缩并上传。
