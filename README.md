...
* **长文优化，质量出众**
    ...
    * 💎 **质量追求**：支持 基础提示、角色介绍、背景设定、翻译风格、提取提示词 等提示词调整，拥有 一键AI润色、一键提取术语 等功能，满足对翻译质量有更高要求的用户。
...
### 3. 📦 打包避坑、闪退根源分析与发布规范 (发布必读)
在以往的打包发布中，由于 PyInstaller 打包机制和项目特殊的目录结构，经常容易出现生成的 EXE **双击毫无反应、瞬间闪退** 或 **点击特定页面时报错崩溃**（例如：`ModuleNotFoundError: No module named 'ModuleFolders.Domain.FileReader.TxtReader'`）。为了彻底杜绝此类问题，请在打包时严格遵守以下规范：

#### ⚠️ 闪退与报错根源分析
1. **动态加载/延迟导入（Lazy Import）与命名空间包（Namespace Package）**
   * **现象**：打包过程非常顺利无报错，但运行时一调用特定文件格式提取（如 Txt/Docx/Epub）或点击特定功能就直接抛出 `ModuleNotFoundError` 闪退。
   * **原因**：
     * PyInstaller 是通过静态分析 AST（语法树）来抓取依赖的。然而本项目为了插件式扩展，许多文件读写器（如 `ModuleFolders/Domain/FileAccessor` 目录下的各类格式处理器）是在运行时根据后缀名通过 `importlib.import_module` 动态加载的。PyInstaller 静态扫描无法感知这些模块的存在。
     * 虽然在打包脚本中我们配置了 `--collect-submodules`，但如果 `ModuleFolders` 及其子目录下**缺失了 `__init__.py` 文件**，Python 会将其归类为“命名空间包”（Namespace Package）。**PyInstaller 在打包时无法通过 `--collect-submodules` 正确扫描并加载命名空间包下的子模块**，导致它们在打包时被完全遗漏。
2. **多分支/副本目录文件物理缺失**
   * **现象**：点击某个新开发的 UI 页面或设置项时程序瞬间崩溃，后台日志报 `ModuleNotFoundError`。
   * **原因**：本项目同时存在主根目录与子分支（如 `AiNiee-latest`）。在子分支里新增的 UI 页面模块（例如 `PromptSettings/ExtractionSettings` 等）如果没有物理同步回主根目录，在主根目录调用打包脚本时，就会缺失这部分物理文件，导致最终的 EXE 缺失关键模块。

#### 📋 准备发布的打包要求与检查清单
在每次正式打包发布前，请务必依次检查并执行以下 5 项规范：
* **核心前提：补全各级目录的空 `__init__.py`**
  * 必须确保 `ModuleFolders` 和 `UserInterface` 的所有子目录（除了 `__pycache__` 等临时目录）下均存在空的 `__init__.py` 文件，将它们规范化为传统的 Python 标准包。否则 PyInstaller 会将其识别为命名空间包，从而跳过对它们动态加载子模块的收集。
* **第一步：多分支物理文件完整性对比**
  * 打包执行前，**必须**对比根目录与开发分支（如 `AiNiee-latest`）下的文件。确保所有新增的 UI 页面（尤其是 `UserInterface` 目录下的子菜单、弹窗组件）、提取模块等已 100% 物理拷贝并同步补全到根目录下，防止因漏拷贝导致物理文件缺失。
* **第二步：动态导入（Hidden Imports）登记**
  * 检查最近是否新增了通过动态 `import_module` 加载的读写器，或者是否使用了带有动态特性的第三方库（如 `chardet` 包含 mypyc 等动态编译管道）。
  * 若有新增，必须在打包脚本 `Tools/pyinstall.py`（或 `AiNiee.spec`）的 `hiddenimports` 参数中显式地将该模块的包路径追加进去。
* **第三步：配置文件脱敏与默认提示词校验**
  * ⚠️ **严禁泄露密钥**：发布前，必须检查打包源目录的 `Resource/config.json`。将 `ApiKey` 键值设为空字符串 `""` 或者是去隐私状态，切勿将测试用的私有 Key 打包进发布版本。
  * **默认提示词同步**：确保最先进的提取提示词（如最新的 PDF 专用中翻英、英翻中提取提示词）已写入默认的 `config.json` 模板中，并随同 `Resource` 目录物理拷贝到打包生成文件夹 of 相应位置，保证用户开箱即用。
* **第四步：控制台命令行烟雾测试**
  * 打包完成后，**切勿直接双击 EXE 完事**。
  * 应该打开命令行（Cmd 或 PowerShell），切换至打包好的目标路径，使用命令行启动程序 `./AiNiee.exe`。
  * 在命令行模式下，依次点击软件内的各个设置页面、导入一份测试文档进行翻译。观察命令行窗口中是否有隐藏的 `ModuleNotFoundError` 警告或静态资源加载报错，确认无任何异常日志后方可进行压缩发布。
