...
【  词义检查  】是原文与译文的词义相似度检测功能，用来判断译文是否翻译错误，以解决AI翻译时错行的问题。开启这个功能会耗费一段时间进行检查，而且会将错误内容进行重翻译，也会增加开销，而非常非常吃性能，CPU利用率会达到100%，请慎重开启。这个功能使用到了huggingface上的开源模型multilingual-MiniLM-L12-v2，第一次使用时会下载500mb的模型文件在C盘的.cache文件夹中，之后使用就不用了。
