...
class ContentLayoutBasicSettingsPage(QFrame, Base):
    ...
    def __init__(self, text: str, window) -> None:
        ...
        # 默认配置
        self.default = {
            "content_layout_target_platform": "deepseek",
            "content_layout_output_path": "./output",
            "content_layout_auto_set_output_path": True,
        }
    ...
    def add_widget_api(self, parent, config) -> None:
        ...
        def update_widget(widget) -> None:
            ...
            widget.set_current_index(max(0, widget.find_text(self.find_name_by_tag(config, config.get("content_layout_target_platform")))))
        ...
        def current_text_changed(widget, text: str) -> None:
            config = self.load_config()
            config["content_layout_target_platform"] = self.find_tag_by_name(config, text)
    ...
    def add_widget_06(self, parent, config) -> None:
        def widget_init(widget):
            info_cont = self.tra("当前输出文件夹为") + f" {config.get("content_layout_output_path")}"
        ...
        def widget_callback(widget) -> None:
            ...
            # 更新并保存配置
            config = self.load_config()
            config["content_layout_output_path"] = path.strip()
            self.save_config(config)
        ...
        def drop_callback(widget, dropped_text) -> None:
            ...
            # 更新并保存配置
            config = self.load_config()
            config["content_layout_output_path"] = dropped_text.strip()
            self.save_config(config)
    ...
    def add_widget_07(self, parent, config) -> None:
        def widget_init(widget) -> None:
            widget.set_checked(config.get("content_layout_auto_set_output_path"))

        def widget_callback(widget, checked: bool) -> None:
            config = self.load_config()
            config["content_layout_auto_set_output_path"] = checked
            self.save_config(config)
