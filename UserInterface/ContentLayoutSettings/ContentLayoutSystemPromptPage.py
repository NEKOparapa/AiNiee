...
# 主界面
class ContentLayoutSystemPromptPage(QFrame, Base):
    ...
    def __init__(self, text: str, window: FluentWindow) -> None:
        ...
        # 默认配置
        self.default = {
            "content_layout_prompt_selection":{"last_selected_id": PromptBuilderEnum.COMMON,"prompt_content": ""},
            "content_layout_user_prompt_data": [],
        }
        ...
        selected_prompt = config.get("content_layout_prompt_selection",{})
        self.user_prompts = config.get("content_layout_user_prompt_data",[])
    ...
    def save_last_selection(self, prompt_id,prompt_content):
        ...
        config = self.load_config()
        config["content_layout_prompt_selection"] = {"last_selected_id": prompt_id,"prompt_content": prompt_content}
        self.save_config(config)

    # 保存新的用户提示词
    def save_user_prompts(self):
        config = self.load_config()
        config["content_layout_user_prompt_data"] = self.user_prompts
        self.save_config(config)
