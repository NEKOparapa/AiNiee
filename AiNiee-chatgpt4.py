...
from qfluentwidgets import InfoBar, InfoBarPosition, NavigationWidget, SpinBox, ComboBox, LineEdit, PrimaryPushButton, PushButton ,StateToolTip, SwitchButton, TextEdit, Theme, setTheme ,isDarkTheme, NavigationInterface,NavigationItemPosition
...
Running_status = 0  #存储程序工作的状态，0是空闲状态，1是正在测试请求状态，2是MTool项目正在翻译状态，3是T++项目正在翻译的状态，10是主窗口退出状态
...
def on_update_signal(str): 
    ...
    if str == "Update_ui" :
        ...
        if Running_status == 2: 
            ...
            Window.Interface2.progressBar.setValue(int(Translation_Progress))
            Window.Interface2.label13.setText(money_used_str + "＄")

        #T++项目正在翻译
        elif Running_status == 3:
            money_used_str = "{:.4f}".format(money_used)  # 将浮点数格式化为小数点后4位的字符串
            Window.Interface3.progressBar2.setValue(int(Translation_Progress))
            Window.Interface3.label23.setText(money_used_str + "＄")
                ...
                for row in ws.iter_rows(min_row=2, min_col=1):  # 从第2行开始遍历每一行
                    ...
                    if key is not None:
                        if key in source:  # 如果key在source字典中
                            value = TS_Backup[key]  # 获取source字典中对应的value
...
def Read_Write_Config(mode):
    ...
    if mode == "write":
        Account_Type = Window.Interface1.comboBox.currentText()      #获取账号类型下拉框当前选中选项的值
        Model_Type =  Window.Interface1.comboBox2.currentText()      #获取模型类型下拉框当前选中选项的值
        Proxy_Address = Window.Interface1.LineEdit1.text()            #获取代理地址
        API_key_str = Window.Interface1.TextEdit2.toPlainText()        #获取apikey输入值

        #如果是MTool界面
        Prompt_Mtool = Window.Interface2.TextEdit.toPlainText()             #获取MTool界面提示词
        Translation_lines_Mtool = Window.Interface2.spinBox1.value()        #获取MTool界面翻译行数
        Semantic_Check_Switch_Mtool = Window.Interface2.SwitchButton1.isChecked()#获取语义检查开关的状态
        #如果是T++界面
        Prompt_Tpp = Window.Interface3.TextEdit.toPlainText()             #获取T++界面提示词
        Translation_lines_Tpp = Window.Interface3.spinBox1.value()        #获取T++界面翻译行数
        Semantic_Check_Switch_Tpp = Window.Interface3.SwitchButton1.isChecked()#获取语义检查开关的状态
...
def On_button_clicked5():
    ...
    if Running_status == 0:
        ...
        if Inspection_results == 0 :  #配置没有完全填写
            ...
            #显示隐藏控件
            Window.Interface2.progressBar.show() 
            Window.Interface2.label12.show()
            Window.Interface2.label13.show() 
...
def On_button_clicked6():
    ...
    if Running_status == 0:
        ...
        if Inspection_results == 0 :  #配置没有完全填写
            ...
            #显示隐藏控件
            Window.Interface3.progressBar2.show() 
            Window.Interface3.label22.show()
            Window.Interface3.label23.show() 
...
def Open_file_Mtool():
    ...
    if file_name:
        ...
        return  # 直接返回，不执行后续操作
    #设置控件里的文本显示
    Window.Interface2.label9.setText(file_name)

# ——————————————————————————————————————————选择项目文件夹（T++）函数——————————————————————————————————————————
def Select_folder_Tpp():
    global Tpp_path
    ...
    if Tpp_path:
        ...
        print('[INFO]  未选择文件夹')
        return  # 直接返回，不执行后续操作
    Window.Interface3.label9.setText(Tpp_path)

# ——————————————————————————————————————————选择输出文件夹函数——————————————————————————————————————————
def Select_folder_Mtool():
    global dir_path
    ...
    if dir_path:
        ...
        print('[INFO]  未选择文件夹')
        return  # 直接返回，不执行后续操作
    Window.Interface2.label11.setText(dir_path)
    Window.Interface3.label11.setText(dir_path)
...
# ——————————————————————————————————————————系统配置函数——————————————————————————————————————————
def Config(num):
    global file_name,dir_path ,Account_Type ,  Prompt, Translation_lines,The_Max_workers, API_key_list,tokens_limit_per,AI_model,Semantic_Check_Switch
    #—————————————————————————————————————————— 读取配置信息——————————————————————————————————————————
    ...
    if num == 1:#如果是MTool界面
        Prompt = Window.Interface2.TextEdit.toPlainText()             #获取提示词
        Translation_lines = Window.Interface2.spinBox1.value()        #获取翻译行数
        Semantic_Check_Switch = Window.Interface2.SwitchButton1.isChecked()#获取语义检查开关的状态
    elif num == 2:#如果是T++界面
        Prompt = Window.Interface3.TextEdit.toPlainText()             #获取提示词
        Translation_lines = Window.Interface3.spinBox1.value()        #获取翻译行数
        Semantic_Check_Switch = Window.Interface3.SwitchButton1.isChecked()#获取语义检查开关的状态
    ...
    if num == 1:#如果是MTool界面
        ...
        if (not API_key_list[0]) or (not Prompt)  or (not Translation_lines) or(not Tpp_path) or(not dir_path)  :
            print("\033[1;31mError:\033[0m 请正确填写配置,不要留空")
            return 0  #返回错误参数
...
# ——————————————————————————————————————————检查没能成功翻译的文本，递减行数翻译————————————————————————————————————————
  ...
  # ——————————————————————————————————————————进行语义相似度检查，并重翻译—————————————————————————————————————————
    ...
    if Semantic_Check_Switch :
        ...
        #遍历result_dict字典，将每个key对应的value存储进sentences[1],并将source_mid字典中对应的key的value存储进sentences[0]
...
class Widget1(QFrame):#自定义的widget内容界面
...
class Widget2(QFrame):#自定义的widget内容界面
    ...
    def __init__(self, text: str, parent=None):#解释器会自动调用这个函数
       ...
       #设置“语义检查”选择开关
        ...
        self.SwitchButton1.checkedChanged.connect(lambda isChecked: self.SwitchButton1.setText('开启' if isChecked else '关闭'))
...
class Widget3(QFrame):#自定义的widget内容界面
    def __init__(self, text: str, parent=None):#解释器会自动调用这个函数
        super().__init__(parent=parent)          #调用父类的构造函数
        self.setObjectName(text.replace(' ', '-'))#设置对象名，作用是在NavigationInterface中的addItem中的routeKey参数中使用
...
class Widget_test(QFrame):#自定义的widget内容界面
...
class AvatarWidget(NavigationWidget):#自定义的头像控件
    """ Avatar widget """
...
class window(FramelessWindow): #自定义的父1窗口
    ...
    def __init__(self):
        ...
        self.Interface1 = Widget1('Interface1', self)     #创建子界面Interface(搜索界面)，传入参数为对象名和parent
        self.Interface2 = Widget2('Interface2', self)      #创建子界面Interface(音乐界面) ，传入参数为对象名和parent
        self.Interface3 = Widget3('Interface3', self)        #创建子界面Interface(视频界面)，传入参数为对象名和parent
        #self.Interface4 = Widget_test('Interface4', self)     #创建子界面Interface(文件夹界面)，传入参数为对象名和parent

        self.stackWidget.addWidget(self.Interface1)  #将子界面添加到父2堆栈窗口中
        self.stackWidget.addWidget(self.Interface2)
        self.stackWidget.addWidget(self.Interface3)
        #self.stackWidget.addWidget(self.Interface4)
    ...
    def initNavigation(self): #详细介绍：https://pyqt-fluent-widgets.readthedocs.io/zh_CN/latest/navigation.html
        self.navigationInterface.addItem(  #addItem函数是导航栏的函数，用于添加导航项
            routeKey=self.Interface1.objectName(), #设置路由键,路由键是导航项的唯一标识符,用于切换导航项,这里设置为子界面的对象名
            icon=FIF.FEEDBACK, #设置左侧图标
            text='账号设置',  #设置显示文本
            onClick=lambda: self.switchTo(self.Interface1) #设置点击事件
        )   #添加搜索导航项，传入参数：路由键，图标，文本，点击事件
        self.navigationInterface.addItem(
            routeKey=self.Interface2.objectName(),
            icon=FIF.BOOK_SHELF,
            text='Mtool项目',
            onClick=lambda: self.switchTo(self.Interface2)
        )  #添加音乐导航项
        self.navigationInterface.addItem(
            routeKey=self.Interface3.objectName(),
            icon=FIF.BOOK_SHELF,
            text='Translator++项目',
            onClick=lambda: self.switchTo(self.Interface3)
        ) #添加视频导航项

        self.navigationInterface.addSeparator() #添加分隔符

        #添加测试导航项
        #self.navigationInterface.addItem(routeKey=self.Interface4.objectName(),icon=FIF.FOLDER,text='测试',onClick=lambda: self.switchTo(self.Interface4),position=NavigationItemPosition.SCROLL) 
       ...
       # 添加头像导航项
        ...
        self.navigationInterface.setDefaultRouteKey(self.Interface1.objectName()) #设置默认的路由键,不起作用
    ...
    def initWindow(self): 
        ...
        self.setWindowTitle('AiNiee-chatgpt') #设置窗口的标题
