...
from PyQt5.QtCore import  QEvent, QObject, QPoint, QRect,  QUrl,  Qt, pyqtSignal #需要安装库 pip3 install PyQt5
from PyQt5.QtWidgets import QApplication, QFrame, QProgressBar, QLabel,QFileDialog,  QStackedWidget, QHBoxLayout

from qfluentwidgets import CheckBox,InfoBar, InfoBarPosition, NavigationWidget, Slider, SpinBox, ComboBox, LineEdit, PrimaryPushButton, PushButton ,StateToolTip, SwitchButton, TextEdit, Theme, ToolTip, setTheme ,isDarkTheme, NavigationInterface,NavigationItemPosition
...
Software_Version = "AiNiee-chatgpt4.20"  #软件版本号
...
# ——————————————————————————————————————————检查没能成功翻译的文本，递减行数翻译————————————————————————————————————————
    ...
    while count_not_Translate != 0 :
        ...
        #如果只剩下10句左右没有翻译则直接逐行翻译
        if count_not_Translate <= 10:
...
class Widget18(QFrame):#自定义的widget内容界面
    def __init__(self, text: str, parent=None):#解释器会自动调用这个函数
        ...
        self._toolTip = ToolTip(parent=self) #创建界面里的提示框，用于给各个控件显示提示框
        ...
        #设置“温度”标签的提示框
        self.label0.setToolTip('✨重新启动软件后，内置参数设置为Tem-0.0 ; Top-1.0 ; Pre-0.5 ; Fre-0.0 ') #设置提示框的文本
        self.label0.setToolTipDuration(5000)  #设置提示框显示的时间
        self.label0.installEventFilter(self)  #安装事件过滤器
        ...
        #设置“温度”标签的提示框
        self.label1.setToolTip('✨默认值是1，已设置为0，取值范围0.0到2.0 ') #设置提示框的文本
        self.label1.setToolTipDuration(3000)  #设置提示框显示的时间
        self.label1.installEventFilter(self)  #安装事件过滤器
        ...
        #设置“top_p”标签的提示框
        self.label4.setToolTip('✨默认值是1，取值范围0.0到1.0 ') #设置提示框的文本
        self.label4.setToolTipDuration(3000)  #设置提示框显示的时间
        self.label4.installEventFilter(self)  #安装事件过滤器
        ...
        #设置“presence_penalty”标签的提示框
        self.label7.setToolTip('✨默认值是0，已设置0.5，取值范围-2.0到2.0 ') #设置提示框的文本
        self.label7.setToolTipDuration(3000)  #设置提示框显示的时间
        self.label7.installEventFilter(self)  #安装事件过滤器
        ...
        #设置“frequency_penalty”标签的提示框
        self.label9.setToolTip('✨默认值是0，取值范围-2.0到2.0 ') #设置提示框的文本
        self.label9.setToolTipDuration(3000)  #设置提示框显示的时间
        self.label9.installEventFilter(self)  #安装事件过滤器
    ...
    def eventFilter(self, obj, e: QEvent): 
        if obj is self:
            return super().eventFilter(obj, e) #如果是窗口本身，就调用父类的事件过滤器

        tip = self._toolTip #获取提示框
        if e.type() == QEvent.Enter:  #如果是鼠标进入事件
            tip.setText(obj.toolTip()) #设置提示框的文本
            tip.setDuration(obj.toolTipDuration()) #设置提示框的显示时间
            tip.adjustPos(obj.mapToGlobal(QPoint()), obj.size()) #调整提示框的位置
            tip.show()
        elif e.type() == QEvent.Leave: #如果是鼠标离开事件
            tip.hide() #隐藏提示框
        elif e.type() == QEvent.ToolTip: #如果是鼠标移动事件
            return True #返回True，表示不显示系统自带的提示框
