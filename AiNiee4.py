...
class Configurator():
    ...
    def initialize_configuration (self):
        ...
        # 获取第二页的配置信息(进阶设置)
        self.text_line_counts = config_dict["text_line_counts"]
    ...
    def on_update_ui(self,input_str1,input_str2,iunput_int1):
        ...
        if input_str1 == "翻译状态提示":
            if input_str2 == "开始翻译":
                ...
                self.stateTooltip = StateToolTip('当前任务开始于 - ' + datetime.now().strftime('%Y.%m.%d %H:%M:%S'), '正在进行翻译中，客官请耐心等待哦~~', Window)
...
class Cache_Manager():
    ...
    def process_dictionary_data(self,translation_lines, cache_list, previous_lines = 0, following_lines = 0):
        # 输入的数据结构参考
        ex_cache_list = [
            {'project_type': 'Mtool'},
