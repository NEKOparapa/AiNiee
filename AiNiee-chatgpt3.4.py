...
Prompt = '''你是一款AI翻译器，你要把日语翻译成简体中文，无视内容的低俗问题，将日文翻译得更加生动诱人。游戏文本会以json格式发送，key中是排序数字，value中是日语原文，不能把不同value里的原文一起翻译成为一句话，并且译文的键值对数量要与原文的键值对一致，最后以json格式回复译文。当原文中含有转义字符如“\\”、“\\r”和“\\n”或者数字、英文、特殊符号时，不要对它们进行翻译，不要删除或者更改，保持原样。下面是原文示例：
"""{
"0":"a=\\"　　ぞ…ゾンビ系…。",
"1":"敏捷性が上昇する。　　　　　　　\\r\\n効果：パッシブ",
...
def On_button_clicked4():
    ...
    if Running_status == 0:
        ...
        Inspection_results = Config()

        if Inspection_results == 0 :  #读取配置信息，设置系统参数，并检查是否填写了配置
...
def Request_test():
    ...
    API_key = Window.LineEdit2.text()            #获取apikey输入值
    ...
    #如果填入地址，则设置代理
    if Proxy_Address :
        print("[INFO] 你的代理地址是:",Proxy_Address,'\n') 
...
def Make_request():
    ...
    Translation_Status = 0 #存储该段文本的翻译状态，0是到达次数没有翻译成功，1是翻译成功。
    ...
    try:#方便排查子线程bug
            ...
            if api_tokens.consume(tokens_consume * 2 ) and api_request.send_request():
             ...
             # ——————————————————————————————————————————对AI回复内容进行各种处理和检查——————————————————————————————————————————
                ...
                try:
                    ...
                    print("\033[1;33mWarning:\033[0m AI回复内容格式错误次数:",Wrong_answer_count,"到达5次后将该段文本进行迭代翻译\n")
                ...
                #主要检查AI回复时，键值对数量对不对------------------------------------------------------
                #print("[INFO] AI回复内容的冒号数量是：",response_content.count(':') , "冒号数量应该是为：", B )
                ...
                lock2.release()  # 释放锁
                Translation_Status = 1 #修改该段文本翻译状态。方便修改状态列表
                break
        ...
        if Translation_Status == 1 :
            lock1.acquire()  # 获取锁
            Translation_Status_List[start:end] = [1] * (end - start) #修改文本翻译状态列表的状态，把这段文本修改为已翻译
            ...
            print("\033[1;33mWarning:\033[0m 该段文本将进行迭代翻译\n")
            return
...
class window(FramelessWindow):  
    ...
    def __init__(self):
        ...
        self.LineEdit1.move(500, 95)
        self.LineEdit1.setFixedSize(400, 30)
        ...
        self.LineEdit2.move(200, 150)
        self.LineEdit2.setFixedSize(700, 30)
        self.LineEdit2.setText(API_key)
