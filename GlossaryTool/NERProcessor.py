...
class NERProcessor(Base):
    ...
    def _load_model(self, model_name: str):
        ...
        with self.nlp_lock:
            ...
            # 尝试查找模型路径，先在上一级目录找，再在当前目录找
            paths_to_check = [
                os.path.join('..', 'Resource', 'Models', 'ner', model_name),
                os.path.join('.', 'Resource', 'Models', 'ner', model_name),
            ]
            ...
            if not model_path:
                self.error(f"模型路径不存在，已尝试: {paths_to_check}")
                return None

            self.info(f"正在加载 spaCy 模型: {model_name}...")

            if model_name.startswith("ja_") or "ja_core" in model_name:
                try:
                    import sudachipy
                except ImportError:
                    self.error(f"加载日语模型 {model_name} 需要 'sudachipy' 库，但未找到。请尝试安装: pip install sudachipy sudachidict-core")
                    return None
    ...
    def _is_compatible_language(self, text: str, model_name: str) -> bool:
        """
        检查文本是否与当前模型兼容。
        主要用于防止英文模型处理大段中文/日文/韩文文本。
        """
        ...
        if model_name.startswith("en"):
            cjk_count = 0
            check_len = min(len(text), 500)
            
            for char in text[:check_len]:
                # 检查 CJK 统一汉字、日文平假名/片假名、韩文音节
                if ('\u4e00' <= char <= '\u9fff' or 
                    '\u3040' <= char <= '\u30ff' or 
                    '\uac00' <= char <= '\ud7af'):
                    cjk_count += 1
            
            # 如果 CJK 字符占比超过 10%，则认为不是英文文本
            if cjk_count / check_len > 0.1:
                return False
                
        return True

    def extract_terms(self, items_data: list, model_name: str, entity_types: list, progress_callback=None) -> list:
        """
        ...
        Args:
            ...
            progress_callback (callable, optional): 进度回调函数，接收 (current, total) 参数。
        ...
        # 增加 max_length 限制，虽然我们通过分片处理，但作为保险
        ...
        for item_data in items_data:
            ...
            try:
                # 预处理文本：分片
                # 使用 5000 字符分片，参考 AiNiee CacheManager 逻辑，保持行完整性
                # spaCy/SudachiPy 对输入长度有严格限制（约49KB），因此这里设置较小的分片大小
                chunks = self._split_text(source_text, max_chunk_size=5000)
                ...
                for chunk in chunks:
                    ...
                    # 检查文本与模型是否兼容
                    if not self._is_compatible_language(chunk, model_name):
                        continue
                    ...
                    for ent in doc.ents:
                        if ent.label_ in entity_types:
                            raw_results.append({
                                "term": ent.text,
                                "type": ent.label_,
                                "context": chunk[:200].replace('\n', ' '), # 只取前200字符作为上下文
                                "file_path": file_path,
                            })
                            
            except Exception as e:
                self.error(f"处理文件 {file_path} 时出错: {e}")
    ...
    def _split_text(self, text, max_chunk_size=5000):
        """
        文本分片方法，参考 AiNiee CacheManager 的逻辑。
        优先按换行符分割，保持行（Item）的完整性。
        """
        ...
        for line in lines:
            ...
            # 如果当前块加上新行会超过限制，且当前块不为空，则提交当前块
            if current_chunk and (current_length + line_len > max_chunk_size):
                chunks.append('\n'.join(current_chunk))
                current_chunk = []
                current_length = 0
            
            # 将行加入当前块（即使单行超过限制，也作为一个独立的块处理，保持完整性）
